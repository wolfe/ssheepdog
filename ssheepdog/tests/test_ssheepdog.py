...
def create_user(**kwargs):
    ...
    if not kwargs.get('ssh_key') and kwargs.get('username'):
        u = kwargs['username']
        pubkey_file = os.path.join(KEYS_DIR, kwargs['username']+".pub")
        defaults['ssh_key'] = read_file(pubkey_file)
...
creator = call_with_defaults(public_key='testpubkey',
                             private_key='testprikey')
create_application_key = creator(ApplicationKey.objects.create)
...
class VagrantTests(TestCase):
    ...
    def reset_vagrant(self):
        with settings(hide('everything', 'status')):
            ...
            for u in ['login']:
                pubkey_file = os.path.join(KEYS_DIR, "ssheepdog.pub")
                run('sudo bash -c "echo %s > ~%s/.ssh/authorized_keys"' % (
                    read_file(pubkey_file).strip(), u))
                run('sudo chown %s:logingroup ~%s/.ssh/authorized_keys'
                    % (u, u))
            disconnect_all()
...
def key_present(user, login):
    return (user.get_profile().formatted_public_key
            in login.get_authorized_keys())
...
class NumQueriesTest(TestCase):
    ...
    def setUp(self):
        ...
        self.logins = [create_login(username="login_%d" % i,
...
class ApplicationKeyTests(TestCase):
    ...
    def test_when_latest_key_differs_from_login_key(self):
        ...
        self.assertTrue(previous.formatted_public_key
